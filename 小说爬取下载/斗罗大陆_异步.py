...
import time
import aiohttp,asyncio,aiomultiprocess
...
def get_bookurl_html(url,bookname):
    ...
    response = requests.get(url=url_booknamne,headers=headers)
    response.encoding=response.apparent_encoding
    html = etree.HTML(response.text)
    return html,url_booknamne

def parsm_save(html,bookname,title):
    content = ''.join(html.xpath('//div[@id="content"]//text()'))
    if not os.path.exists(bookname):
        os.mkdir(bookname)
    with open(f'{bookname}/{title}.txt','w',encoding='utf-8') as f:
        f.write(content)
        f.close()

def prase_url_title(book_name):
...
async def get_content(url,headers,title):
    html = await get_html(url,headers)
    parsm_save(html,book_name,title)
    print(f"{book_name}:\t", f"url:{url},章节名:{title}")

starttime = datetime.now()
book_name = input('请输入你想要下载的小说名字')


title,url = prase_url_title(book_name)


for i in range(len(title)):
    get_headers()
    corontinue = get_content(url[i],headers,title[i])
